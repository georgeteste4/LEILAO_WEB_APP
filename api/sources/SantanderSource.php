...
/**
 * Driver Especializado de Extração para o Banco Santander (Leilões e Imóveis Retomados).
 * URL Base: https://www.santanderimoveis.com.br
 */
class SantanderSource implements SourceInterface
{
    private string $slug = 'santander';
    private string $nome = 'Banco Santander (Imóveis & Leilões)';
    private string $urlBase = 'https://www.santanderimoveis.com.br';
    private string $channelUrl = 'https://www.portalzuk.com.br/leilao-de-imoveis/v/banco-santander';
    ...
    public function scrapeImoveis(Scraper $scraper, array $filtros): array
    {
        ...
        // 1. Tentar buscar no canal do vendedor
        ...
        if ($html && strlen($html) > 200) {
            $imoveis = $this->parseSantanderHtml($html, $uf, $filtros);
        }
        ...
        if (empty($imoveis) && !empty($uf)) {
            ...
            if ($htmlEstado && strlen($htmlEstado) > 200) {
                $imoveisEstado = $this->parseSantanderHtml($htmlEstado, $uf, $filtros, true);
    ...
    public function scrapeDetalhes(Scraper $scraper, string $url): array
    {
        return [
            ...
            'nome_leiloeiro' => 'Banco Santander',
            'link_leiloeiro' => $this->urlBase,
        ];
    }

    private function parseSantanderHtml(string $html, string $targetUf, array $filtros, bool $filterByBankInText = false): array
    {
        $doc = new DOMDocument();
        ...
        foreach ($allLinks as $a) {
            ...
            $cardUf = '';
            $cidade = 'São Luís';
            $endereco = 'Consulte o edital Santander';
            ...
            // Se for busca na listagem do estado, verificar se pertence ao Santander
            if ($filterByBankInText) {
                if (stripos($cardText, 'santander') === false && stripos($href, 'santander') === false) {
                    continue;
                }
            }
            ...
            $titulo = "{$tipoImovel} Banco Santander em {$cidade}/{$cardUf}";
            ...
            $imoveis[] = [
                ...
                'desconto' => $desconto,
                'modalidade' => 'Leilão Oficial Santander',
                'data_encerramento' => 'Consulte edital Santander',
                'data_inclusao' => date('d/m/Y'),
                'edital' => $href,
                'link_matricula' => $href,
                'numero_matricula' => null,
